...
const DashboardContactsAllContacts = () => {
  ...
  const handleDeleteContacts = (id) => {
    ...
    if (confirmDelete) {
      axios.delete(`https://quentin.hugoorickx.tech/contact/${id}`)
        .then(res => {
            ...
            <tbody>
              {displayedContacts.map(contact => (
                <tr key={contact.ids}>
                  ...
                  <td><button className="delete" onClick={() => handleDeleteContacts(contact.id)}></button></td>
      <td><Link to={`/editcontactspage/${contact.id}`}><button className="edit"></button></Link></td>
                </tr>
              ))}
            </tbody>
          </table>
